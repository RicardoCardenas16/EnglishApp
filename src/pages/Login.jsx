...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            // Normalize username/email to lowercase to avoid multi-account confusion
            const normalizedEmail = email.toLowerCase().trim();
            const userToUse = normalizedEmail === 'admin@example.com' ? 'admin' : normalizedEmail;
        ...
        <div className="bg-background-light dark:bg-background-dark min-h-screen flex items-center justify-center font-display">
            <div className="relative flex h-full min-h-screen w-full max-w-[430px] flex-col bg-white dark:bg-background-dark overflow-x-hidden shadow-2xl">
                ...
                <div className="flex items-center bg-white dark:bg-background-dark p-4 justify-between">
                    <div className="text-[#111318] dark:text-white flex size-12 shrink-0 items-center justify-start cursor-pointer transition-transform active:scale-90" onClick={() => setIsRegister(!isRegister)}>
                        <span className="material-symbols-outlined text-2xl">{isRegister ? 'login' : 'person_add'}</span>
                    </div>
                ...
                <div className="flex flex-col items-center px-6 pt-8 pb-4">
                    <div className="mb-6 relative group">
                        <img
                            src="/logo.jpg"
                            alt="Chori Logo"
                            className="w-24 h-24 rounded-3xl shadow-xl object-cover border-4 border-white dark:border-gray-800 transition-transform group-hover:scale-105"
                        />
                        <div className="absolute -bottom-2 -right-2 size-8 bg-primary rounded-full flex items-center justify-center text-white border-4 border-white dark:border-gray-800">
                    ...
                    <p className="text-[#616f89] dark:text-gray-400 text-sm font-normal leading-relaxed text-center px-4">
                        {isRegister ? 'Join our community and start your journey today.' : 'Welcome back! Continue mastering your English skills.'}
                        <br />
                        <span className="text-[10px] opacity-30 mt-2 block">V1.0.8 - Session Persistence</span>
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 py-4">
                    ...
                    {isRegister && (
                        <label className="flex flex-col w-full animate-fade-in">
                    ...
                    {/* Main Action */}
                    <div className="pt-2">
                        <button type="submit" className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2 group">
                            <span>{isRegister ? 'Create Account' : 'Sign In'}</span>
                            <span className="material-symbols-outlined group-hover:translate-x-1 transition-transform">
                                {isRegister ? 'person_add' : 'logout'}
                            </span>
                        </button>
                    </div>
                </form>
                ...
                <div className="flex items-center px-6 py-6">
                    ...
                    <span className="px-4 text-xs text-[#616f89] dark:text-gray-400 font-bold uppercase tracking-widest">Or continue with</span>
                    <div className="flex-grow border-t border-[#dbdfe6] dark:border-gray-700"></div>
                </div>
